...
class AssessmentService:
    ...
    def get_user_records(self, user_id: int) -> List[models.AssessmentRecord]:
        ...
        return (
            ...
            .filter(models.AssessmentRecord.assessment_type != "learning")
    ...
    def build_detail_response(self, record: models.AssessmentRecord) -> Dict:
        ...
        if record.rom_data:
            ...
            for k, v in rd.items():
                item = {
                    "joint": k,
                    "side": "left" if "left" in k else ("right" if "right" in k else "bilateral")
                }
                if isinstance(v, dict):
                    item.update(v)
                else:
                    # 兼容旧数据：ROM 值直接是数值而非对象
                    item["value"] = v
                rom_items.append(item)
            response["rom_analysis"] = rom_items
