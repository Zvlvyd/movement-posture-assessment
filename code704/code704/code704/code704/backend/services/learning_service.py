...
class LearningService:
    ...
    def get_report(self, record_id: int) -> Optional[Dict]:
        """获取训练报告数据"""
        import json
        record = self._db.query(models.AssessmentRecord).filter(
        ...
        rom_data = json.loads(record.rom_data) if record.rom_data else {}
        movement_data = json.loads(record.movement_data) if record.movement_data else {}
        muscle_findings = json.loads(record.muscle_findings) if record.muscle_findings else {}
        report_data = json.loads(record.report_data) if record.report_data else {}
        return {
            "id": record.id,
            "action_name": posture_data.get("action", ""),
            "view": posture_data.get("view", ""),
            "overall_score": round(record.overall_score, 1) if record.overall_score else 0,
            "best_score": round(rom_data.get("best_score", 0), 1),
            "frame_count": rom_data.get("frame_count", 0),
            "rep_count": rom_data.get("rep_count", 0),
            "duration": report_data.get("duration", 0),
            "summary": report_data.get("summary", []),
            "feedback_counts": muscle_findings.get("feedback_counts", {}),
            "angle_history": movement_data.get("angle_history", []),
...
HOLD_EXERCISES = {'平板支撑', '侧平板', '反向平板', '单腿臀桥', '门框胸拉伸', '膝盖平板支撑', '双腿闭眼站立', '单腿站立', '闭眼单腿站立', '最伟大拉伸', '髋部打开', '小狗伸展式', '胸部打开', '肩部拉伸'}
...
_FSM_REQUIRED_ANGLES = {
    ...
    'dead_bug': ['left_hip', 'right_hip'],
    'heel_toe_rock': ['trunk_tilt'],
    'hip_opener': ['left_hip', 'right_hip'],
    'extended_puppy': ['left_shoulder', 'right_shoulder'],
    'chest_open': ['left_shoulder', 'right_shoulder'],
    'thoracic_rotation': ['left_shoulder', 'right_shoulder'],
    'shoulder_stretch': ['left_shoulder', 'right_shoulder'],
}
...
def _get_fsm_key(action_name: str):
    ...
    mapping = [
        ...
        ('小狗伸展式', 'extended_puppy'),
        ('胸部打开', 'chest_open'),
        ('侧卧胸椎旋转', 'thoracic_rotation'),
        ('肩部拉伸', 'shoulder_stretch'),
        ('墙壁俯卧撑', 'pushup'),
        ('上斜俯卧撑', 'pushup'),
        ('扶椅深蹲', 'squat'),
        ('俯卧Y字举', 'prone_y_raise'),
...
class RealtimeLearningService(BaseWebSocketHandler):
    ...
    async def handle_session(self, ws: WebSocket, user_id: int):
        ...
        ema_score = None          # EMA 平滑分数（非对称：快升慢降）
        score_floor_active = False  # 达到50分后不再跌回50以下
        best_smoothed = 0.0       # 平滑后的最佳分
        smoothed_sum = 0.0        # 平滑分数累加（用于算均值）
        smoothed_count = 0        # 平滑分数帧数
        # 自动完成 / 人体检测 状态
        good_streak = 0
            ...
            while True:
                ...
                # ── frame ──────────────────────────────
                    ...
                    if kp_xy is None:
                        ...
                        session_phase = "waiting_for_body"
                        # hold 计时：人离开了暂停计时器
                        if exercise_type == "hold" and hold_start_time is not None:
                            now = time.time()
                            hold_total_seconds += now - hold_start_time
                            hold_start_time = None
                        await ws.send_json({
                            "type": "comparison",
                            "frame": frame_count,
                    ...
                    if fsm is not None and fsm_key and _fsm_angles_valid(fsm_key, user_angles):
                        ...
                        # ── 兜底：人物短暂离开后返回，FSM 仍在 holding 但计时器未恢复 ──
                        # 人物离开时 valid_kp_count 不足 → 跳过 FSM 更新 → hold_start_time 被清空
                        # 人物回来后 FSM 未发生转换（仍在 holding）→ update() 返回 None → 计时器未重启
                        # 因此需要在此处独立检查：FSM 处于 hold 有效状态但计时器未启动时，重新开始计时
                        if exercise_type == "hold" and _is_hold_quality_state(fsm_state) and hold_start_time is None:
                            hold_start_time = time.time()
                            logger.debug("[Learning] hold timer resumed after person returned (state=%s, total=%.1fs)",
                                        fsm_state, hold_total_seconds)
                    ...
                    if frame_count <= 3:
                        logger.info("[Learning] frame=%d score=%s best_score=%s angles=%s diffs=%s",
                                     frame_count, score, best_score,
                                     list(user_angles.keys())[:5] if user_angles else [],
                                     len(comparison.get("diffs", [])))

                    if score is not None:
                        total_score_sum += score
                        # 首次捕获或分数更高时更新最佳帧数据
                        is_first_capture = (not best_frame_diffs and not best_frame_angles)
                        if is_first_capture or score > best_score:
                            if score > best_score:
                                best_score = score
                    ...
                    # EMA 平滑策略：
                    #   上升：快速响应（α=0.6，一帧从70→88）
                    #   合理下降（≥20分）：缓慢衰减（α=0.06）
                    #   极低分（<20，用户未做动作）：冻结，不更新
                    #   达到50分后底限锁定在50分
                    LOW_FREEZE = 20
                    FLOOR_SCORE = 50
                    if ema_score is None:
                        ema_score = float(score) if score is not None else 0.0
                    elif score is not None and score >= ema_score:
                        ema_score = 0.6 * score + 0.4 * ema_score  # 快升（一帧可见）
                    elif score is not None and score >= LOW_FREEZE:
                        ema_score = 0.06 * score + 0.94 * ema_score  # 慢降
                    # else: score < 20 → freeze, don't update ema_score
                    if ema_score is not None and ema_score >= FLOOR_SCORE:
                        score_floor_active = True
                    if score_floor_active and ema_score is not None:
                        ema_score = max(float(FLOOR_SCORE), ema_score)
                    smoothed = round(ema_score, 1) if ema_score is not None else None
                    # 只在底限激活后（首次达到50分）才累加，排除训练前的低分阶段
                    if score_floor_active and smoothed is not None:
                    ...
                    if frame_count <= 3 or frame_count % 60 == 0:
                        logger.info("[EMA] frame=%d raw=%s smoothed=%s floor=%s",
                                     frame_count, round(score, 1) if score else 'None', smoothed, score_floor_active)
                    ...
                    if good_streak >= AUTO_COMPLETE_STREAK:
                        ...
                        summary = self._generate_summary(
                            best_smoothed, feedback_counts, common_errors, duration,
                        )
                        ...
                        record_id = self._save_learning_record(
                            ...
                            (smoothed_sum / max(smoothed_count, 1) if smoothed_count > 0 else best_smoothed), best_smoothed, frame_count,
                        ...
                        await ws.send_json({
                            ...
                            "record_id": record_id,
                            "total_score": round(smoothed_sum / max(smoothed_count, 1), 1) if smoothed_count > 0 else best_smoothed,
                            "best_score": best_smoothed,
                            "duration": duration,
                ...
                # ── finish ─────────────────────────────
                    ...
                    logger.info("[Learning] finish: best_score=%s, best_frame_diffs=%s, best_frame_angles=%s",
                                best_score, len(best_frame_diffs) if isinstance(best_frame_diffs, list) else type(best_frame_diffs),
                                len(best_frame_angles) if isinstance(best_frame_angles, dict) else type(best_frame_angles))

                    # 保存到数据库
                    avg_score = round(smoothed_sum / max(smoothed_count, 1), 1) if smoothed_count > 0 else best_smoothed
                    record_id = self._save_learning_record(
                        user_id, current_action, current_view,
                        avg_score, best_smoothed, frame_count,
                        angle_history, feedback_counts, summary, duration,
                        rep_count=rep_count,
                    )
        ...
        for joint_key, standard in standard_angles.items():
            ...
            if user_val is None:
                diffs.append({
                    ...
                    "standard_max": standard.get("max", 180),
            ...
            diffs.append({
                ...
                "diff": diff,
                "status": status,
                "percent": score,  # 范围适配的百分比（good=100, close=70, warning=40, bad=10）
            })
    ...
    def _generate_summary(
        ...
        if best_score >= 85:
            summary.append(f"最佳得分 {round(best_score)} 分，动作质量优秀！继续保持。")
        elif best_score >= 70:
            summary.append(f"最佳得分 {round(best_score)} 分，整体表现良好，部分细节可优化。")
        elif best_score >= 50:
            summary.append(f"最佳得分 {round(best_score)} 分，存在动作偏差，建议针对性改进。")
        elif best_score > 0:
            summary.append(f"最佳得分 {round(best_score)} 分，动作规范性有待提高，建议从基础要领开始练习。")
        else:
