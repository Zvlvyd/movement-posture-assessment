...
from backend.database.models_v2 import PrescriptionPlan, PrescriptionPlanItem, TrainingCompletion
...
def _apply_items(db: Session, plan: PrescriptionPlan, items_data: List[dict]):
    ...
    # Delete child TrainingCompletion records first (FK constraint)
    item_ids = [item.id for item in (plan.items or [])]
    if item_ids:
        db.query(TrainingCompletion).filter(
            TrainingCompletion.plan_item_id.in_(item_ids)
