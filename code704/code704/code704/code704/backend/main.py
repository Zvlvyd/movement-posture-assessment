...
# ── 将 Ultralytics 配置放到项目内的可写目录 ──
# 默认的 %APPDATA%/Ultralytics 在沙箱、服务账号和部分部署环境中可能不可写。
import os
_project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
_ultralytics_config_dir = os.path.join(_project_root, ".ultralytics")
os.makedirs(_ultralytics_config_dir, exist_ok=True)
os.environ.setdefault("YOLO_CONFIG_DIR", _ultralytics_config_dir)
os.environ.setdefault("ULTRALYTICS_CONFIG_DIR", _ultralytics_config_dir)
try:
...
from backend.routers.student_class import router as student_class_router
