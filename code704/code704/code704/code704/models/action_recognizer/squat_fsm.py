...
def create_lunge_fsm() -> StateMachine:
    ...
    fsm.add_state('complete')
    fsm.add_transition('standing', 'lunging', lambda ctx: ctx.get('front_knee_angle', 180.0) < 140.0)
    fsm.add_transition('lunging', 'bottom', lambda ctx: ctx.get('front_knee_angle', 180.0) < 85.0)
    fsm.add_transition('bottom', 'recovering', lambda ctx: ctx.get('front_knee_angle', 180.0) > 105.0)
    fsm.add_transition('recovering', 'complete', lambda ctx: ctx.get('front_knee_angle', 180.0) > 160.0)
    fsm.add_transition('complete', 'standing', lambda ctx: ctx.get('front_knee_angle', 180.0) < 130.0)
    return fsm
...
def create_plank_fsm() -> StateMachine:
    """标准平板支撑：正视角髋角 ~145-185°，塌腰暂停，恢复继续"""
    fsm = StateMachine('plank')
    fsm.add_state('ready')
    fsm.add_state('holding')
    fsm.add_state('drooping')
    fsm.add_transition('ready', 'holding', lambda ctx: 145.0 <= (ctx.get('hip_angle', 180.0)) <= 185.0)
    fsm.add_transition('holding', 'drooping', lambda ctx: ctx.get('hip_angle', 180.0) < 140.0 or ctx.get('hip_angle', 180.0) > 190.0)
    fsm.add_transition('drooping', 'holding', lambda ctx: 145.0 <= ctx.get('hip_angle', 180.0) <= 185.0)
    return fsm

def create_knee_plank_fsm() -> StateMachine:
    """膝盖平板支撑：侧视髋角实测 ~100-120°，塌腰暂停，恢复继续"""
    fsm = StateMachine('knee_plank')
    fsm.add_state('ready')
    fsm.add_state('holding')
    fsm.add_state('drooping')
    fsm.add_transition('ready', 'holding', lambda ctx: 85.0 <= (ctx.get('hip_angle', 180.0)) <= 120.0)
    fsm.add_transition('holding', 'drooping', lambda ctx: ctx.get('hip_angle', 180.0) < 80.0 or ctx.get('hip_angle', 180.0) > 130.0)
    fsm.add_transition('drooping', 'holding', lambda ctx: 85.0 <= ctx.get('hip_angle', 180.0) <= 120.0)
    return fsm
...
def create_cat_cow_fsm() -> StateMachine:
    """猫牛式：四足跪姿，脊柱拱起(猫, 髋角~110°) ↔ 下沉(牛, 髋角~140°) = 1 个循环"""
    fsm = StateMachine('cat_cow')
    fsm.add_state('neutral')
    fsm.add_state('cat')
    fsm.add_state('cow')
    fsm.add_state('complete')
    fsm.add_transition('neutral', 'cat', lambda ctx: ctx.get('hip_angle', 180) < 122.0)
    fsm.add_transition('cat', 'cow', lambda ctx: ctx.get('hip_angle', 180) > 135.0)
    fsm.add_transition('cow', 'complete', lambda ctx: ctx.get('hip_angle', 180) < 125.0)
    fsm.add_transition('complete', 'neutral', lambda ctx: ctx.get('hip_angle', 180) < 115.0)
    return fsm
...
def create_bridge_fsm() -> StateMachine:
    ...
    fsm.add_state('lowering')
    fsm.add_state('complete')
    fsm.add_transition('down', 'lifting', lambda ctx: ctx.get('hip_angle', 90) > 148.0)
    fsm.add_transition('lifting', 'up', lambda ctx: ctx.get('hip_angle', 90) > 160.0)
    fsm.add_transition('up', 'lowering', lambda ctx: ctx.get('hip_angle', 90) < 155.0)
    fsm.add_transition('lowering', 'complete', lambda ctx: ctx.get('hip_angle', 90) < 144.0)
    fsm.add_transition('lowering', 'up', lambda ctx: ctx.get('hip_angle', 90) > 160.0)
    fsm.add_transition('complete', 'down', lambda ctx: ctx.get('hip_angle', 90) > 152.0)
    fsm.add_transition('complete', 'up', lambda ctx: ctx.get('hip_angle', 90) > 160.0)
    return fsm
...
def create_dead_bug_fsm() -> StateMachine:
    """死虫式：仰卧交替伸展四肢（次数类）。侧卧视角髋角实测 130~180°，伸腿时骤降至 ~130°，收腿回升至 ~170°"""
    fsm = StateMachine('dead_bug')
    fsm.add_state('ready')
    fsm.add_state('extending')
    fsm.add_state('extended')
    fsm.add_state('complete')
    fsm.add_transition('ready', 'extending', lambda ctx: ctx.get('hip_angle', 180) < 162.0)
    fsm.add_transition('extending', 'extended', lambda ctx: ctx.get('hip_angle', 180) < 142.0)
    fsm.add_transition('extended', 'complete', lambda ctx: ctx.get('hip_angle', 180) > 162.0)
    fsm.add_transition('complete', 'extending', lambda ctx: ctx.get('hip_angle', 180) < 148.0)
    return fsm


def create_wall_angel_fsm() -> StateMachine:
    """靠墙天使：手臂贴墙上滑至Y字、下滑回W字（次数类）。侧视角 shoulder_angle 从~15°升到~145°再降回"""
    fsm = StateMachine('wall_angel')
    fsm.add_state('bottom')
    fsm.add_state('ascending')
    fsm.add_state('top')
    fsm.add_state('descending')
    fsm.add_state('complete')
    fsm.add_transition('bottom', 'ascending', lambda ctx: ctx.get('shoulder_angle', 180) > 35.0)
    fsm.add_transition('ascending', 'top', lambda ctx: ctx.get('shoulder_angle', 180) > 130.0)
    fsm.add_transition('top', 'descending', lambda ctx: ctx.get('shoulder_angle', 180) < 125.0)
    fsm.add_transition('descending', 'complete', lambda ctx: ctx.get('shoulder_angle', 180) < 30.0)
    fsm.add_transition('complete', 'ascending', lambda ctx: ctx.get('shoulder_angle', 180) > 35.0)
    return fsm
...
def get_fsm_context(action_name: str, angles: dict) -> dict:
    ...
    if name == 'squat':
        ...
        ctx['hip_angle'] = angles.get('left_hip') if angles.get('left_hip') is not None else (angles.get('right_hip') if angles.get('right_hip') is not None else 180.0)
    elif name in ('doorway_stretch', 'shoulder_circle', 'prone_y_raise', 'prone_t_raise', 'wall_angel'):
        ...
        # 髋关节环绕是双侧交替动作，取双侧髋角中较小的（更活跃的一侧）
        lh = angles.get('left_hip')
        rh = angles.get('right_hip')
        if lh is not None and rh is not None:
            ctx['hip_angle'] = min(lh, rh)
        elif lh is not None:
            ctx['hip_angle'] = lh
        elif rh is not None:
            ctx['hip_angle'] = rh
        ...
        ctx['hip_angle'] = angles.get('left_hip') if angles.get('left_hip') is not None else (angles.get('right_hip') if angles.get('right_hip') is not None else 90.0)
    else:
        ctx['knee_angle'] = angles.get('left_knee') if angles.get('left_knee') is not None else (angles.get('right_knee') if angles.get('right_knee') is not None else 180.0)
    return ctx
...
def get_fsm_for_action(action_name: str) -> StateMachine:
    fsms = {
        ...
        'dead_bug': create_dead_bug_fsm,
        'knee_plank': create_knee_plank_fsm,
        'wall_angel': create_wall_angel_fsm,
    }
