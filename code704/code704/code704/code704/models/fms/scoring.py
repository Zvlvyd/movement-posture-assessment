...
class FMSScoringEngine:
    ...
    @staticmethod
    def _score_knee_flexibility(knee_angle: float) -> Score:
        """深蹲灵活性评分（膝盖角度，° — 实时/视频筛查路径使用）
        ...
        膝盖弯曲角度越小 → 柔韧性越好 → 分数越高
         180° = 站直没蹲 → 0 分
          ...
          60° = 极限深蹲 → 100 分
        """
        s = max(0, min(100, (180.0 - knee_angle) / 1.2))
        return Score("flexibility", round(s, 1), f"过头深蹲 膝盖角度 {knee_angle:.0f}°")
