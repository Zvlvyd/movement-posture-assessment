...
def process_video(input_path, model_path='yolov8s-pose.pt', output_path='视频_结果_s.mp4', export_frames=10):
    print(f"=== 使用 {model_path} 处理视频 ===")
    ...
    engine = YOLOPoseEngine(model_path=model_path, device='cpu')
    print("✅ 模型加载成功")
    ...
    with open('视频关键点数据_s.json', 'w', encoding='utf-8') as f:
        json.dump({
            'model': model_path,
            'video_info': {
                'width': width,
    ...
    print("✅ 关键点数据已导出到 视频关键点数据_s.json")

if __name__ == '__main__':
    process_video('视频.mp4', model_path='yolov8s-pose.pt')