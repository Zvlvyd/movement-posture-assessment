...
import {
  ...
  Statistic, Row, Col, Tabs, Progress, Descriptions, Empty, Spin
} from "antd";
import {
...
export default function CoachPage() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      api.get("/coach/summary").then(r => setSummary(r.data)),
      api.get("/coach/classes").then(r => setClasses(r.data)),
  ...
  const loadClassDetail = async (classId: number) => {
    ...
    try {
      const [statsRes, trendRes] = await Promise.all([
        api.get(`/coach/classes/${classId}/stats`),
        api.get(`/coach/classes/${classId}/trend`, { params: { days: 14 } }),
      ]);
  ...
  const createClass = async () => {
    ...
    try {
      ...
      const r = await api.get("/coach/classes");
      setClasses(r.data);
    } catch { message.error("创建失败"); }
  };

  const addStudent = async () => {
    if (!addClassId || !addStudentId) { message.warning("请填写班级ID和学员ID"); return; }
    try {
      await api.post(`/coach/classes/${addClassId}/students`, null, {
        params: { student_id: Number(addStudentId) },
      });
      message.success("学员已添加");
      setAddModal(false); setAddStudentId("");
      if (selectedClassId === addClassId) loadClassDetail(addClassId);
    ...
    return (
      <div>
        ...
        <Card title="学员进度" size="small" loading={detailLoading} extra={
          <Button size="small" icon={<PlusOutlined />} onClick={() => { setAddClassId(selectedClassId); setAddModal(true); }}>添加学员</Button>
        }>
    ...
    <div>
      ...
      <Card title="班级管理" extra={
        <Space>
          ...
          <Button onClick={() => { setAddClassId(null); setAddModal(true); }}>添加学员</Button>
        </Space>
      }>
        {classes.length === 0 ? (
      ...
      <Modal title="添加学员" open={addModal} onOk={addStudent} onCancel={() => setAddModal(false)}>
        <Input placeholder="班级ID（可选，留空则不指定班级）" type="number"
               style={{ marginBottom: 8 }}
               value={addClassId ?? ""}
               onChange={e => setAddClassId(e.target.value ? Number(e.target.value) : null)} />
        <Input placeholder="学员ID" type="number" value={addStudentId}
