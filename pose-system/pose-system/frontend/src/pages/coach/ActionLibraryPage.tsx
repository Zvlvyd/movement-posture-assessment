...
import {
  ...
  ReloadOutlined, DeleteOutlined, EyeInvisibleOutlined,
...
export default function ActionLibraryPage() {
  ...
  }, [page, pageSize, search, filterCategory, filterFamily, filterDifficulty, bodyPart, showHidden]);
  ...
  // Delete custom action (soft delete)
  const handleDelete = (action: UnifiedAction) => {
    if (!action.id) return;
    Modal.confirm({
      title: '确认删除',
      content: '删除后，该动作将从学员学习库和训练计划中移除，但历史处方不受影响。',
      okText: '确认删除',
      okType: 'danger',
      cancelText: '取消',
      onOk: async () => {
  ...
  const handleToggleVisibility = async (action: UnifiedAction) => {
    const identifier = action.id ? String(action.id) : action.name;
    const newVisible = action.is_visible === false;
    try {
      await coachApi.setActionVisibility(identifier, newVisible);
      message.success(newVisible ? '已取消隐藏' : '已隐藏');
      fetchActions();
    } catch (e: any) {
  ...
  const renderActionCard = (action: UnifiedAction) => (
    <Col key={action.id || action.json_id || action.name} xs={24} sm={12} md={8} lg={6}
      style={{ opacity: action.is_visible === false ? 0.5 : 1 }}>
      <Card
        hoverable
        size="small"
        ...
        actions={[
          ...
          action.is_visible !== false ? (
            <Tooltip title="隐藏" key="hide">
              <EyeInvisibleOutlined onClick={() => handleToggleVisibility(action)} />
            </Tooltip>
          ) : (
            <Tooltip title="取消隐藏" key="unhide">
              <EyeOutlined style={{ color: '#faad14' }} onClick={() => handleToggleVisibility(action)} />
            </Tooltip>
          ),
          action.is_custom && action.is_visible !== false ? (
            <Tooltip title="删除" key="delete">
              <DeleteOutlined style={{ color: '#ff4d4f' }} onClick={() => handleDelete(action)} />
            </Tooltip>
          ) : null,
        ].filter(Boolean)}
      >
        ...
        <Col xs={24} md={6} lg={5}>
          <Card size="small" title={<span><FilterOutlined /> 筛选</span>}>
            ...
            <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
              <Switch checked={showHidden} onChange={v => { setShowHidden(v); setPage(1); }} size="small" />
              <Text type="secondary" style={{ fontSize: 12 }}>显示已隐藏</Text>
            </div>
          </Card>
        </Col>
