      ...
      <Card title={<Space><ExperimentOutlined />FMS 功能运动能力筛查</Space>}>
        ...
        {currentTest >= 0 && currentTestData && (
          <div>
            ...
            {/* 准备阶段：动作示范 + 准备按钮 / 倒计时 */}
            {testPhase !== 'completed' && showPreparing && (
              <div>
                <MovementDemo
            ...
            {/* 无人检测警告 */}
            {noPersonWarning && testPhase !== 'completed' && (
              <div style={{
            ...
            <div style={{
              ...
              {testPhase === 'completed' && testScore !== null && (
                <div style={{
                  ...
                  <div style={{ textAlign: 'center', color: '#fff', padding: 32, maxWidth: 400 }}>
                    <TrophyOutlined style={{ fontSize: 48, color: testScore >= 60 ? '#52c41a' : '#faad14', marginBottom: 16 }} />
                    ...
                    <div style={{ fontSize: 56, fontWeight: 700, marginBottom: 8,
                      color: testScore >= 70 ? '#52c41a' : testScore >= 40 ? '#faad14' : '#f5222d' }}>
                      {testScore} 分
                    </div>
                    <div style={{ marginBottom: 16 }}>
                      <Tag color={testScore >= 70 ? 'green' : testScore >= 40 ? 'orange' : 'red'} style={{ fontSize: 14, padding: '4px 16px' }}>
                        {testScore >= 70 ? '良好' : testScore >= 40 ? '一般' : '需加强'}
                      </Tag>
                    </div>
                    {testMessage && <Typography.Text style={{ color: 'rgba(255,255,255,0.7)', display: 'block', marginBottom: 24 }}>{testMessage}</Typography.Text>}
                    {currentTest < 4 ? (
            ...
            {/* 控制卡片 */}
            {testPhase !== 'completed' && <Card size="small" style={{ marginTop: 16 }}>
              <Space direction="vertical" style={{ width: '100%' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <Space size={8}>
                    ...
                    <Button size="small" icon={<ForwardOutlined />} onClick={skipTest}
                      disabled={testPhase === 'skipped'}>
                      跳过此项
                    </Button>
                  </Space>
                </div>
                {/* 计时类动作：保持时间 */}
                {(currentTest === 0 || currentTest === 3) && testPhase === 'running' && (
                  <div style={{ textAlign: 'center', marginBottom: 4 }}>
                    <Typography.Text style={{ fontSize: 32, fontWeight: 700, fontFamily: 'var(--font-mono)', color: 'var(--color-accent)' }}>
                ...
                {testPhase === 'running' && (
                  noPersonWarning
                    ...
                    : (currentTest !== 0 && currentTest !== 3) && <Spin indicator={<CameraOutlined style={{ fontSize: 24 }} spin />} />
                )}
                {testScore !== null && (
