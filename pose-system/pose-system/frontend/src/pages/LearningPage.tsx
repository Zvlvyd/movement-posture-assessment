...
export default function LearningPage() {
  ...
  const [sessionState, sessionActions] = useTrainingSession({
    ...
    initialView: demoAction?.views?.[0] || "正面",
    onComplete: (data) => {
      setResult(data);
      setMode("result");
    },
  ...
  // 合并 learnable(JSON标准角度) + DB动作(教练上传)，标注是否可实时对比
  const allMerged = useMemo(() => {
    const dbByName = new Map(actions.map(a => [a.name, a]));
    const seen = new Set<string>();
    const result: (ActionItem & { hasStandardAngles?: boolean })[] = [];

    // 1. learnable 动作（有标准角度的优先）
    for (const l of learnable) {
      seen.add(l.name);
      const db = dbByName.get(l.name);
      result.push({
        id: db?.id || 0,
        name: l.name,
        category: l.category || db?.category || "",
        difficulty: l.difficulty || db?.difficulty || 1,
        description: db?.description || l.description || "",
        target_body_parts: (l.target_body_parts || []).join(", "),
        hasStandardAngles: l.has_standard_angles,
      });
    }

    // 2. DB独有动作（教练上传，暂无标准角度）
    for (const a of actions) {
      if (!seen.has(a.name)) {
        result.push({
          ...a,
          hasStandardAngles: false,
        });
      }
    }
  ...
  const filtered = useMemo(() => {
    ...
    return [...list].sort((a, b) => {
      const aHas = a.hasStandardAngles;
      const bHas = b.hasStandardAngles;
      if (aHas && !bHas) return -1;
      if (!aHas && bHas) return 1;
      return ((a as any).difficulty ?? 99) - ((b as any).difficulty ?? 99);
    });
  }, [allMerged, category, family, search, learnableMap]);
      ...
      {loading ? (
        ...
        <List
          ...
          renderItem={(item: ActionItem) => {
            ...
            const hasAngles = (item as any).hasStandardAngles ?? l?.has_standard_angles ?? false;
            const isLearnable = !!l || hasAngles;
            return (
              <List.Item>
                <Card
                  hoverable
