...
export const coachApi = {
  ...
  trainingRecords: (studentId: number, days?: number) =>
    api.get(`/coach/students/${studentId}/training-records`, { params: { days: days || 30 } }).then(r => r.data),
  ...
  getChangeRequest: (crId: number) =>
    api.get(`/coach/change-requests/${crId}`).then(r => r.data),
  approveChangeRequest: (crId: number) =>
    api.put(`/coach/change-requests/${crId}/approve`).then(r => r.data),
  adjustChangeRequest: (crId: number, items: any[], coachNotes?: string) =>
    api.put(`/coach/change-requests/${crId}/adjust`, { items, coach_notes: coachNotes || '' }).then(r => r.data),
  rejectChangeRequest: (crId: number, coachNotes: string) =>
    api.put(`/coach/change-requests/${crId}/reject`, { coach_notes: coachNotes }).then(r => r.data),
};

// ── Messages ───────────────────────────────────────────────
export const messageApi = {
  conversations: (page = 1) =>
    api.get("/messages", { params: { page } }).then(r => r.data),
  unreadCount: () =>
    api.get("/messages/unread-count").then(r => r.data),
  getWith: (partnerId: number) =>
    api.get(`/messages/with/${partnerId}`).then(r => r.data),
  send: (receiverId: number, content: string, relatedType?: string, relatedId?: number) =>
    api.post("/messages", null, { params: { receiver_id: receiverId, content, related_type: relatedType, related_id: relatedId } }).then(r => r.data),
...
export const planEditApi = {
  ...
  submitChange: (planId: number, notes?: string) =>
    api.post(`/prescription-v2/plans/${planId}/submit-change`, { student_notes: notes || '' }).then(r => r.data),
  myChangeRequests: () =>
    api.get("/prescription-v2/change-requests").then(r => r.data),
};

// ── Student Class ──────────────────────────────────────────
export const studentClassApi = {
  join: (invite_code: string) =>
    api.post("/class/join", { invite_code }).then(r => r.data),
  myClasses: () =>
    api.get("/class/my-classes").then(r => r.data),
  classDetail: (id: number) =>
    api.get(`/class/my-classes/${id}`).then(r => r.data),
  leaveClass: (id: number) =>
    api.delete(`/class/my-classes/${id}/leave`).then(r => r.data),
};
...
export const prescriptionV2Api = {
  ...
  getProgress: (planId: number) =>
    api.get<{plan_id:number; items:{plan_item_id:number; sets_done:number; total_sets:number; latest_reps:number; latest_score:number}[]; completed_item_ids:number[]; total_items:number}>(
      "/prescription-v2/plans/" + planId + "/progress"
    ).then(r => r.data),
  getActions: () =>
