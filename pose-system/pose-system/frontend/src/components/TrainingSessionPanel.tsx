...
export default function TrainingSessionPanel({
  ...
  // 前端直接根据动作名判断是否为静态保持类（无需等待后端消息）
  const HOLD_KEYWORDS = ['平板支撑', '侧平板', '反向平板', '单腿臀桥', '死虫式', '靠墙天使', '侧卧股四头肌拉伸', '婴儿式', '门框胸拉伸', '眼镜蛇式', '膝盖平板支撑', '膝盖侧平板', '髋屈肌半跪拉伸', '坐姿梨状肌拉伸', '双腿闭眼站立', '侧平板抬腿', '单腿平板支撑', '单腿站立', '闭眼单腿站立', '最伟大拉伸'];
  const isHoldExercise = HOLD_KEYWORDS.some(kw => (exerciseInfo?.name || '').includes(kw));
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 80px)", padding: "0 16px 16px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0" }}>
        <Space>
          ...
          {/* 计时/计数显示 */}
          {isHoldExercise
            ? (<Tag color="var(--color-accent)" style={{ fontSize: 15, padding: "4px 14px", fontWeight: 700, fontVariantNumeric: "tabular-nums" }}>{formatTime(holdTime)}</Tag>)
            : fsmState
              ? (<Tag color="var(--color-accent)" style={{ fontSize: repCount > 0 ? 15 : 13, padding: repCount > 0 ? "4px 12px" : "2px 10px", fontWeight: 700 }}>{repCount} 次</Tag>)
              : null
          }
          {bestScore > 0 && (
      ...
      <Row gutter={16} style={{ flex: 1, overflow: "hidden" }}>
          ...
          <div style={{
            ...
            {isSessionActive && (
              <div style={{ position: "absolute", top: 12, left: 12, display: "flex", gap: 8, alignItems: "center" }}>
                ...
                {fsmState && (
                  isHoldExercise ? (
                    <Tag color="var(--color-accent)" style={{ fontWeight: 700 }}>
                      {formatTime(holdTime)}
                    </Tag>
                  ) : (
                    <Tag color="var(--color-accent)" style={{ fontWeight: 700 }}>
        ...
        <Col span={8} style={{ height: "100%", overflow: "auto" }}>
          {exerciseInfo && (
            <Card size="small" style={{ marginBottom: 12 }}>
              <Descriptions size="small" column={1}>
                ...
                {isHoldExercise ? (
                  <Descriptions.Item label="保持时长">
                    <Text strong style={{ color: "var(--color-accent)", fontSize: 18 }}>{formatTime(holdTime)}</Text>
                  </Descriptions.Item>
                ) : (
                  repCount > 0 && (
                    <Descriptions.Item label="已完成次数">
                      <Text strong style={{ color: "var(--color-accent)", fontSize: 18 }}>{repCount}</Text>
                    </Descriptions.Item>
                  )
                )}
          ...
          <Card size="small" title={
            <Space>
              <span>实时反馈</span>
              {feedbacks.filter(f => f.severity !== 'info').length > 0 &&
                <Tag color="error">{feedbacks.filter(f => f.severity !== 'info').length}</Tag>}
            </Space>
          }>
            {(() => {
              const warnings = feedbacks.filter(f => f.severity === 'warning' || f.severity === 'bad');
              const infos = feedbacks.filter(f => f.severity === 'info');

              if (warnings.length > 0) {
                // 有需要纠正的问题
                return warnings.map((fb, i) => (
                  <div key={i} style={{
                    padding: "8px 12px", marginBottom: 6, borderRadius: 6,
                    background: fb.severity === "bad" ? "rgba(245,34,45,0.1)" : "rgba(250,140,22,0.1)",
                    border: `1px solid ${fb.severity === "bad" ? "rgba(245,34,45,0.2)" : "rgba(250,140,22,0.2)"}`,
                    fontSize: 13,
                  }}>
                    <WarningOutlined style={{
                      color: fb.severity === "bad" ? "#f5222d" : "#fa8c16",
                      marginRight: 6,
                    }} />
                    {fb.message}
                  </div>
              ...
              if (infos.length > 0) {
                // 角度接近标准，显示关节点数
                return (
                  <div style={{ padding: 12, textAlign: "center", color: "#faad14" }}>
                    <WarningOutlined style={{ fontSize: 24, marginBottom: 8 }} />
                    <div>已检测到关节 {validKpCount}/17，请全身进入摄像头</div>
                  </div>
                );
              }
