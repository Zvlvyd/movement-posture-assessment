...
class TrainingCompletion(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(Integer, ForeignKey("user.id"), nullable=False, index=True)
    plan_id = Column(Integer, ForeignKey("prescription_plan.id"), nullable=False, index=True)
    plan_item_id = Column(Integer, ForeignKey("prescription_plan_item.id"), nullable=False, index=True)
    action_name = Column(String(200), nullable=False)

    best_score = Column(Float, default=0, comment="最佳得分")
    rep_count = Column(Integer, default=0, comment="完成次数（次数类动作）")
    hold_time_seconds = Column(Float, default=0, comment="保持时长秒（计时类动作）")
    duration_seconds = Column(Float, default=0, comment="训练时长秒")
